...
def add_train_args(parser):
    ...
    parser.add_argument('--use_loss_accel', action='store_true', default=False)

    parser.add_argument('--lambda_accel', type=float, default=1.0)

    parser.add_argument('--guidance_scale', type=float, default=2.0)

    parser.add_argument('--load_num', type=int, default=-1)
...
def config_parse(args):
    ...
    config.trainer.save_freq = max(1, int(config.trainer.epoch // 100))
    ...
    if 'debug' not in config.name:
        config.save = f'{args.save}/{args.name}_{data_prefix}'
    else:
        config.save = f'{args.save}/{args.name}'
